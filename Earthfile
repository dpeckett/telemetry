...
package:
  ...
  RUN mkdir -p /workspace/golang-github-dpeckett-telemetry
  WORKDIR /workspace/golang-github-dpeckett-telemetry
  COPY . .
  ...
  RUN VERSION=$(git describe --tags --abbrev=0 | tr -d 'v') \
    && tar -czf ../golang-github-dpeckett-telemetry_${VERSION}.orig.tar.gz .
